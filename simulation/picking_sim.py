...
import torch
import wandb
from environments.d3il.envs.gym_picking_env.gym_picking.envs.picking import Block_Pick_Env
...
train_contexts = np.load(sim_framework_path("environments/dataset/data/picking/train_contexts.pkl"),
                         allow_pickle=True)

test_contexts = np.load(sim_framework_path("environments/dataset/data/picking/test_contexts.pkl"),
                        allow_pickle=True)
...
class Picking_Sim(BaseSim):
    ...
    def eval_agent(self, agent, contexts, n_trajectories, successes, mean_distance, pid, cpu_set):
        ...
        for context in contexts:
            ...
            for i in range(n_trajectories):
                ...
                while not done:
                    ...
                    obs = np.concatenate((pred_action[:7], obs))
                ...
                # print(f"Mode type:{ type(info['mode'])}")
    ...
    def test_agent(self, agent):
        ...
        if self.n_cores > 1:
            ...
            self.eval_agent(agent, contexts, self.n_trajectories_per_context, successes, mean_distance, 0, set([0]))
        ...
        return successes,  mean_distance